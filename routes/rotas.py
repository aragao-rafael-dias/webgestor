...
""")
SQL_KM_TOTAL_ARBITRADO = text("""
    SELECT km_total_arbitrado
    FROM semed.rotas_memoriais
    WHERE LOWER(BTRIM(codigo_rota)) = LOWER(BTRIM(:codigo_rota))
    ORDER BY id
    LIMIT 1
...
CAMPOS_MEMORIAL = (
    "numero_linha", "linha", "km_ida", "km_volta", "km_total_arbitrado",
    "turnos_ativos", "horarios", "tipo_veiculo", "quantidade_veiculos",
    "onibus_pcd", "inicio", "termino", "rede_ensino", "redes_ensino",
    "localizacao", "areas", "intermunicipal", "assistente_mobilidade",
    "assistente_nome", "veiculo_placa", "motorista", "contato",
    "escolas_atendidas", "observacao", "responsavel_tecnico", "crea",
    "executora",
)
CAMPOS_JSON_MEMORIAL = {
    "turnos_ativos", "horarios", "redes_ensino", "areas", "escolas_atendidas"
}
TURNOS_VALIDOS = ("MANHÃ", "TARDE", "NOITE", "INTEGRAL")


def _aplicar_regra_km_total() -> None:
    """Aplica o KM total automático ou o valor arbitrado, quando informado."""
    if getattr(memorial_routes, "_regra_km_total_arbitrado_aplicada", False):
        return
    ...
    def montar_contexto_com_km_total(registros, dados, pontos_normalizados):
        contexto = montar_contexto_original(registros, dados, pontos_normalizados)
        ...
        quantidade_veiculos = memorial_routes.inteiro_seguro(
            contexto.get("quantidade_veiculos"),
            0,
        )
        quantidade_calculo = quantidade_veiculos if quantidade_veiculos > 0 else 1
        km_total_arbitrado = memorial_routes.decimal_seguro(
            dados.get("km_total_arbitrado")
        )

        if km_total_arbitrado is not None:
            contexto["total_geral"] = km_total_arbitrado
        else:
            contexto["total_geral"] = total_turnos * Decimal(quantidade_calculo)

        contexto["km_total_arbitrado"] = km_total_arbitrado
        return contexto

    def tabela_quilometragem_com_km_total(contexto, estilos):
        elementos = tabela_quilometragem_original(contexto, estilos)
        total_geral = (
            memorial_routes.decimal_seguro(
                contexto.get("total_geral"),
                Decimal("0"),
    ...
    memorial_routes.montar_contexto = montar_contexto_com_km_total
    memorial_routes.tabela_quilometragem = tabela_quilometragem_com_km_total
    memorial_routes._regra_km_total_arbitrado_aplicada = True


_aplicar_regra_km_total()
...
def _km_total_arbitrado_existente(codigo_rota: str) -> float | None:
    ...
    if "km_total_arbitrado" not in colunas:
        return None

    valor = db.session.execute(
        SQL_KM_TOTAL_ARBITRADO,
...
def _normalizar_metadados_recebidos(rota: dict[str, Any]) -> dict[str, Any]:
    ...
    numero_linha = _texto(
        rota.get("numero_linha") or origem.get("numero_linha"),
        100,
    )
    ...
    if quantidade in (None, ""):
        ...
        try:
            ...
            raise ValueError(
                "Quantidade de veículos precisa ser um número inteiro."
            ) from exc
    ...
    horarios_origem = (
        origem.get("horarios")
        if isinstance(origem.get("horarios"), dict)
        else {}
    )
    ...
    redes = [
        item.upper()
        for item in _lista_textos(origem.get("redes_ensino"), 10)
    ]
    areas = [
        item.upper()
        for item in _lista_textos(origem.get("areas"), 10)
    ]
    ...
    return {
        ...
        "km_total_arbitrado": _numero_opcional(
            origem.get("km_total_arbitrado"),
            "KM TOTAL arbitrado",
        ),
        ...
        "assistente_mobilidade": _booleano_opcional(
            origem.get("assistente_mobilidade")
        ),
        ...
        "escolas_atendidas": _lista_textos(
            origem.get("escolas_atendidas"),
            100,
        ),
...
def _salvar_metadados_memorial(codigo_rota: str, dados: dict[str, Any]) -> None:
    ...
    if faltantes:
        raise ValueError(
            ...
            "Execute 'flask db upgrade'. Campos ausentes: "
            + ", ".join(faltantes)
        )
    ...
    if quantidade == 1:
        ...
        valores = [
            ":codigo_rota",
            *[expressoes[campo] for campo in CAMPOS_MEMORIAL],
        ]
        comando = text(
...
def preview_importacao_memorial():
    ...
    try:
        limite_bytes = int(
            current_app.config.get("MEMORIAL_MAX_BYTES", 10 * 1024 * 1024)
        )
        if (
            request.content_length
            and request.content_length > limite_bytes + 1024 * 1024
        ):
            return jsonify({
                "sucesso": False,
                "erro": "O memorial excede o limite de 10 MB.",
            }), 413
        ...
        dados["metadados"]["km_total_arbitrado"] = (
            _km_total_arbitrado_existente(importado.nome_rota)
        )
        ...
        if caminho is not None:
            try:
                ...
                current_app.logger.warning(
                    "Não foi possível remover o DOCX temporário: %s",
                    caminho,
                )
...
def salvar_importacao_memorial():
    ...
    if alertas and not bool(dados.get("confirmar_alertas")):
        return jsonify({
            "sucesso": False,
            "erro": (
                "A prévia possui alertas de geometria. "
                "Confirme a revisão antes de salvar."
            ),
        }), 409
    ...
    if not trechos:
        return jsonify({
            "sucesso": False,
            "erro": "O memorial não possui trechos para importação.",
        }), 400
    ...
    try:
        ...
        for nome_trecho in ("IDA", "VOLTA"):
            trecho = trechos.get(nome_trecho) or {}
            ...
            if len(pontos) < 2 or not geometria:
                preservados.append({
                    "id": existente,
                    "trecho": nome_trecho,
                    "acao": "preservado" if existente is not None else "não criado",
                    "motivo": "trecho sem pontos suficientes para formar uma linha",
                })
                continue

            pontos_json = pontos_para_json(pontos)
            geom_json = validar_geojson_linha(geometria, pontos=pontos)
            parametros = {
                "nome_rota": nome_rota,
                "regiao": regiao,
                "trecho": nome_trecho,
            ...
            if validacao["srid"] != 4326 or not validacao["valida"]:
                raise ValueError(
                    f"A geometria de {nome_trecho} não passou na validação PostGIS."
                )
        ...
        if not alterados:
            raise ValueError(
                "Nenhum trecho possui pontos suficientes para formar uma linha. "
                "Os dados do memorial não foram salvos."
            )
        ...
        return jsonify({
            "sucesso": True,
            "mensagem": (
                f"{nome_rota} e os dados do memorial foram salvos com sucesso."
            ),
            "registros": alterados,
        ...
        return jsonify({
            "sucesso": False,
            "erro": (
                "O banco recusou a atualização. "
                "Nenhum trecho ou dado do memorial foi salvo."
            ),
        }), 500
